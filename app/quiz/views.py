...
class ThemeAddView(AuthRequiredMixin, View):
    ...
    async def post(self):
        ...
        existing_theme = await self.store.quiz.get_theme_by_title(title)
        if existing_theme:
            raise HTTPConflict()
...
class QuestionAddView(AuthRequiredMixin, View):
    ...
    async def post(self):
        ...
        theme = await self.store.quiz.get_theme_by_id(theme_id)
        if not theme:
            raise HTTPNotFound(text="Theme not found")
        
        existing_question = await self.store.quiz.get_question_by_title(title)
        ...
        question = await self.store.quiz.create_question(
            title=title,
            theme_id=theme_id,
            answers=answers
        )
